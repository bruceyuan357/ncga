...
| **Dual-track auth** | Set `NCGA_AUTH_TOKEN=<random 32+ chars>` env. All `POST /api/*` then require one of two tracks. **Browser SPA**: an HMAC-SHA256-signed session cookie (`ncga_sess`, HttpOnly, SameSite=Lax, 30-day max-age) minted via `POST /api/login` (token-for-cookie exchange) and refreshed on already-authenticated `GET /`; the raw token is never injected into HTML — only a no-secret `<meta name="ncga-auth-mode">` marker. Revocation via an in-memory set + `POST /api/logout`; rotating the token invalidates all cookies at once (it is the signing key). **API / extension / scripts**: `Authorization: Bearer <token>` header. |
